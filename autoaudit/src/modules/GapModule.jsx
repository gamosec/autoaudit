...
function sPrompt(results, org, industry, lang) {
  ...
  const gaps = results.filter(r=>r.verdict==="Non-Compliant").slice(0,5).map(r=>r.id).join(", ");
...
export default function GapModule({ t, isRTL, lang }) {
  ...
  if (!orgForm) {
    return <OrgSetup draft={draft} setDraft={setDraft} onStart={() => setOrgForm({...draft})} t={t} isRTL={isRTL} lang={lang} />;
  }
  ...
  return (
    <div style={{maxWidth:520}} dir={isRTL?"rtl":"ltr"}>
      ...
      <button onClick={onStart} disabled={!valid}
        style={{width:"100%",padding:14,background:valid?"linear-gradient(135deg,#10b981,#059669)":"#1e293b",border:"none",borderRadius:11,color:valid?"#fff":"#475569",fontSize:14,fontWeight:800,cursor:valid?"pointer":"default",fontFamily:"inherit"}}>
...
function Assessment({ form, onReset, t, isRTL, lang }) {
  ...
  const addMsg = (role, text, meta={}) => setMessages(m => [...m, {role,text,...meta}]);
  ...
  const submitAnswer = async () => {
    ...
    setPendingFile(null);
    setLoading(true);
    addMsg("assistant", lang==="ar" ? "جاري التقييم..." : "Evaluating...", {isStatus:true});
    ...
    let updatedResults;
    setResults(r => { updatedResults = [...r, newResult]; return updatedResults; });
    ...
    if (next >= TOTAL) {
      ...
      setTimeout(async () => {
        ...
        addMsg("assistant", (lang==="ar"?"اكتمل التقييم! 93 ضابطاً.\n\n":"Assessment complete! All 93 controls.\n\n") + sum);
        setLoading(false);
      }, 200);
    } else {
      setCtrlIdx(next);
          ...
          <div style={{padding:"12px 16px",background:"#0f172a",borderTop:"1px solid #1a2744"}}>
            {pendingFile && (
              <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:8,padding:"6px 10px",background:"#10b98115",border:"1px solid #10b98130",borderRadius:7}}>
                ...
                <button onClick={()=>setPendingFile(null)} style={{marginLeft:"auto",background:"none",border:"none",color:"#ef4444",cursor:"pointer",fontSize:16}}>x</button>
              </div>
            )}
